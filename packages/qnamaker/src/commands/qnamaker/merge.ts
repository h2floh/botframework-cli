...
import {sortQnA, sortAlterations, sortQnAId, mergeQnA} from './../../utils/qnamakerinstanceutils'
...
export default class QnamakerMerge extends Command {
  static description = 'Merges .qna files to into one .qna file while preserving qids.'

  static flags: flags.Input<any> = {
    in: flags.string({char: 'i', description: 'Source .qna file(s)'}),
    ...
    help: flags.help({char: 'h', description: 'qnamaker:merge help'}),
  }
  ...
  async run() {
    try {
      const {flags} = this.parse(QnamakerMerge)
      ...
      if (isQnA) {
        ...
        if(flags.alterations) {
          ...
          sortFunction = sortAlterations
        } else {
          QnA = new KB(file.parseJSON(qnaContent, 'QnA'))
          sortFunction = sortQnAId
        }
